...
/**
 * Class responsible for deserializing and serializing byte array data stored in RMS. 
 * 
 * @author mark voit
 */
public class ByteArrayStorableResource extends AbstractRMSResource {
     ...
     * Constructor of this class.
     * 
     * @param sessionId id of the session this resource is valid for, zero means always valid
     * @param id id of the resource
    ...
    public void deserialize(byte[] bytes) {
	    ...
	    try {
			...
			try {
				if (dataOut != null) ((InputStream)dataOut).close();
			} catch (IOException e) {
			}
			try {
    ...
    public byte[] serialize() {
	    ...
	    try {
			...
			try {
				if (dataOut != null) ((OutputStream)dataOut).close();
			} catch (IOException e) {
			}
			try {
				if (out != null) out.close();
